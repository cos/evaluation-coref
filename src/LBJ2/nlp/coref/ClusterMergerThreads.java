 ...
 **/
public class ClusterMergerThreads implements Comparable<ClusterMergerThreads> {
	/**
	 ...
	 *            as input.
	 **/
	public ClusterMergerThreads(Document d, Classifier c) {
		this(d, c, null);
	}
	 ...
	 **/
	public ClusterMergerThreads(Document d, Classifier c, Filter f) {
		if (f == null)
			filter = new Filter();
		else
			filter = f.clone(this);
	 ...
	 * more.
	 **/
	public ClusterMergerThreads next() {
		if (clusterNames == null || j >= clusterNames.length)
			return null;
		ClusterMergerThreads result = new ClusterMergerThreads();
	 ...
	 **/
	public int compareTo(ClusterMergerThreads c) {
	...
	public static void main(String[] args) {
		...
		final CorefAction[] leafTasks = new CorefAction[numberOfParsers];
		...
		for(int i=0;i<leafTasks.length;i++) {
			...
			leafTasks[i] =  new CorefAction(parser, initialDoc, i);			
		}
		...
		Thread[] t = new Thread[leafTasks.length];
		for(int i=0;i<leafTasks.length;i++) {
			final int j = i;
...
//		final CorefAction[] leafTasksA = leafTasks.getArray();
		...
		for(int i=0;i<leafTasks.length;i++) {
			totalP[0] += leafTasks[i].getTotalP();
			totalR[0] += leafTasks[i].getTotalR();
			totalMentions[0] += leafTasks[i].getTotalMentions();
			results.addAll(leafTasks[i].getResults());
		};
	...
	/**
	 * This method returns a new {@link ClusterMergerThreads.Filter} object that is
	 ...
	 *            {@link ClusterMergerThreads.Filter}, <code>"size"</code> to create a
	 *            {@link ClusterMergerThreads.SizeFilter}, <code>"b3D"</code> to create
	 *            a {@link ClusterMergerThreads.B3DistanceFilter}, or
	 *            <code>"clusterD"</code> to create a
	 *            {@link ClusterMergerThreads.ClusterDistanceFilter}.
	 * @param argument
	 *            Passed to the constructors of
	 *            {@link ClusterMergerThreads.B3DistanceFilter} and
	 *            {@link ClusterMergerThreads.ClusterDistanceFilter}, and ignored by
	 *            other filters.
	 **/
	public static Filter getUnattachedFilter(String specifier, double argument) {
		...
		if (specifier.equals("none"))
			filter = new ClusterMergerThreads().new Filter();
		else if (specifier.equals("size"))
			filter = new ClusterMergerThreads().new SizeFilter();
		else if (specifier.equals("b3D"))
			filter = new ClusterMergerThreads().new B3DistanceFilter(argument);
		else if (specifier.equals("clusterD"))
			filter = new ClusterMergerThreads().new ClusterDistanceFilter(argument);
		else {
	...
	public class Filter {
		...
		protected Filter clone(ClusterMergerThreads merger) {
