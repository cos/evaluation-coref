...
public class SentenceSplitter extends LineByLine
{
  ...
  protected String getParagraph()
  {
    ...
    for (line = readLine(); line != null && line.trim().equals("");
         line = readLine())
      paragraph += line + "\n";

    for (; line != null && !line.trim().equals(""); line = readLine())
      paragraph += line + "\n";

    if (line != null) paragraph += line + "\n";
    return paragraph;
  }
  ...
  public Object next()
  {
    if (sentences.size() == 0)
    {
      ...
      if (!paragraph.trim().equals("")) process(paragraph);
  ...
  {
    for (String paragraph = getParagraph(); !paragraph.trim().equals("");
  ...
  {
    if (paragraph.trim().equals("")) return;
    Matcher m = Pattern.compile("([^-\\s]-\n\\s*|\\S)+").matcher(paragraph);
    ...
    for (int i = 0; i < words.length; ++i)
    {
      ...
      if (punctuationIndex != -1)
      {
        ...
        if (boundary(punctuationIndex, words[i], next1, next2))
        {
          sentences.add(
  ...
  protected boolean boundary(int index, Word word, Word next1, Word next2)
  {
    ...
    if (suffix.form.equals(""))
    {
      if (startsWithQuote(next1) || startsWithOpenBracket(next1)) return true;

      if (next1.form.equals("-RBR-") && next2.form.equals("--"))
        return false;

      if (isClosingBracket(next1)) return true;

      if (prefix.form.equals("") && next1.form.equals("."))
        return false;

      if (next1.form.equals(".")) return true;

      if (next1.form.equals("--") && next2.capitalized
          && endsWithQuote(prefix))
        return false;

      if (next1.form.equals("--")
            && (next2.capitalized || startsWithQuote(next2)))
        return true;
  ...
  /**
    ...
    *
    * @param w  The word in question.
    * @return   <code>true</code> if and only if the argument represents
    *           either a closing bracket or a closing quote.
   **/
  ...
  /**
    ...
    *
    * @param w  The word in question.
    * @return   <code>true</code> if and only if the argument starts with one
    *           of the varieties of open quote named above.
   **/
  ...
  /**
    ...
    *
    * @param w  The word in question.
    * @return   <code>true</code> if and only if the argument starts with any
    *           of the varieties of open bracket named above.
   **/
  ...
  /**
    ...
    *
    * @param w  The word in question.
    * @return   <code>true</code> if and only if the argument matches any of
    *           the above time zone abbreviations.
   **/
  ...
  /**
    ...
    * @param w  The word in question.
    * @return   <code>true</code> if and only if the argument matches any of
    *           the above terminal abbreviations.
   **/
