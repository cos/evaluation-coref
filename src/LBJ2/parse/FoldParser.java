...
/**
  ...
  * depending on the selected splitting strategy.  A particular fold is then
...
public class FoldParser implements Parser
{
  ...
  /**
    * Used only by the random splitting strategy to remember which example
  ...
  /**
    ...
    * in the data only saves computation when the splitting strategy is either
   ...
   **/
  public FoldParser(Parser parser, int K, SplitStrategy split, int pivot,
                    boolean f)
  {
  ...
  /**
    ...
    * can only be used when the splitting strategy is manual.  Using a
    * constructor that allows specification of the number of examples in the
    * data only saves computation when the splitting strategy is either
    * sequential or random.
    *
    * @param parser The parser whose examples are being filtered.
    * @param split  The way in which examples are partitioned into folds.
    ...
    * @param f      Whether to extract examples from the pivot.
   **/
  public FoldParser(Parser parser, SplitStrategy split, int pivot, boolean f)
  {
                    ...
                    boolean f, int e)
  {
    ...
    if (examples == -1
        && (splitStrategy == SplitStrategy.sequential
    ...
    if (splitStrategy == SplitStrategy.random)
    {
    ...
    if (splitStrategy == SplitStrategy.manual)
    {
      this.K = 1;
      for (Object example = parser.next(); example != null;
           example = parser.next())
  ...
  public void reset()
  {
    ...
    if (splitStrategy == SplitStrategy.sequential
        || splitStrategy == SplitStrategy.random)
    {
    ...
    if (splitStrategy == SplitStrategy.random) shuffleIndex = lowerBound;
    if (splitStrategy == SplitStrategy.manual) fold = 0;
    exampleIndex = 0;
  }
  ...
  protected boolean filter(Object example)
  {
    ...
    if (splitStrategy == SplitStrategy.kth)
      return fromPivot == (exampleIndex % K == pivot);
    // splitStrategy == SplitStrategy.manual
  ...
  protected void increment(Object example)
  {
    ...
    {
      if (splitStrategy == SplitStrategy.manual) ++fold;
    }
    else
    {
      if (splitStrategy == SplitStrategy.random)
  ...
  **/
  public static class SplitStrategy
  {
    ...
    /**
      * Represents the split strategy in which every k<sup>th</sup> example is
      * part of the same fold.
    **/
    public static final SplitStrategy kth = new SplitStrategy(2);
    /**
      * Represents the split strategy in which the user manually inserts fold
      * separation objects.
    **/
    ...
    private SplitStrategy(int i) { index = i; }
