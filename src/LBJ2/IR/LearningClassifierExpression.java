...
public class LearningClassifierExpression extends ClassifierExpression
{
  ...
  /**
    * (&oslash;) A Boolean indicating whether feature vectors are to be
    * pre-extracted and stored in a file (if <code>true</code>), or if feature
    * extraction should be done on the fly; argument to
    * <code>preExtract</code>.
   **/
  ...
  /**
    ...
    *   <li>
    *     A numerical attribute, named Dan, and a string attribute, named
    *     Nick: "num_Dan:str_Nick:"
    *   <li>
    *     A numerical attribute, named Dan, a string attribute, named Nick,
    *     and a nominal attribute named Arindam which can take the values
    *     "Cool", "Uncool", or "Kinda Cool":
  ...
  public LearningClassifierExpression(ClassifierExpression l,
                                      ...
                                      Block pb, Expression e, Constant k,
                                      FoldParser.SplitStrategy s,
    ...
    for (Iterator I = cl.iterator(); I.hasNext(); )
    {
      ...
      else if (c.type == Clause.CVAL)
      {
        if (c.argument != null)
          ...
          // crossvalidation gets invoked anyway, this occurs legally in the
          // case of manual separation
          K = new Constant("-1");

        // handle default action
        if (c.splitStrategy != null)
          splitStrategy = c.splitStrategy;
        else
          splitStrategy = FoldParser.SplitStrategy.sequential;
  ...
  public int hashCode()
  {
    ...
    if (splitStrategy != null) result += splitStrategy.hashCode();
    if (testingMetric != null) result += testingMetric.hashCode();
    result += alpha.hashCode();
    if (preExtract != null) result += preExtract.hashCode();
  ...
  public boolean equals(Object o)
  {
    ...
    return (labeler == null && c.labeler == null
             || labeler != null && labeler.equals(c.labeler))
           && extractor.equals(c.extractor)
           && (parser == null && c.parser == null
               || parser != null && parser.equals(c.parser))
           && (rounds == null && c.rounds == null
               || rounds != null && rounds.equals(c.rounds))
           && (learnerConstructor == null && c.learnerConstructor == null
               || learnerConstructor != null
                  && learnerConstructor.equals(c.learnerConstructor))
           && (learnerName == null && c.learnerName == null
               || learnerName != null && learnerName.equals(c.learnerName))
           ...
           && (evaluation == null && c.evaluation == null
               || evaluation != null && evaluation.equals(c.evaluation))
           && (K == null && c.K == null
               || K != null && K.equals(c.K))
           && (splitStrategy == null && c.splitStrategy == null
               || splitStrategy != null
                  && splitStrategy.equals(c.splitStrategy))
           && (testingMetric == null && c.testingMetric == null
               || testingMetric != null
                  && testingMetric.equals(c.testingMetric))
           && alpha.equals(c.alpha)
           && (preExtract == null && c.preExtract == null
               || preExtract != null && preExtract.equals(c.preExtract))
           && (progressOutput == null && c.progressOutput == null
               || progressOutput != null
  ...
  public ASTNodeIterator iterator()
  {
    ...
    // learnerName and learnerParameterBlock are intentionally omitted here so
    // that they can be handled specially by SemanticAnalysis.
    if (evaluation != null) children.add(evaluation);
    if (K != null) children.add(K);
    if (testingMetric != null) children.add(testingMetric);
    children.add(alpha);
    if (preExtract != null) children.add(preExtract);
  ...
  public void write(StringBuffer buffer)
  {
    ...
    if (K != null)
    {
      ...
      if (splitStrategy != FoldParser.SplitStrategy.manual)
      {
        K.write(buffer);
        buffer.append(" ");
      }

      buffer.append(splitStrategy.getName());
  ...
  public StringBuffer shallow()
  {
    ...
    /* These lines are commented because we don't want a change in a
     * preExtract or progressOutput clause to cause retraining.
  ...
  **/
  public static class Clause
  {
    ...
    public static final String[] typeNames =
      new String[]
      {
        "using", "from", "with", "evaluate", "cval", "preExtract",
        "progressOutput", "testingMetric", "alpha"
      };
    ...
    **/
    ...
    /**
      ...
      *           split strategy.
    **/
    public Clause(int t, ASTNode a, Constant r)
    {
      type = t;
      ...
      if (t == CVAL)
      {
        if (r == null)
        {
          if (!(a instanceof Constant
                && ((Constant) a).value.equals("\"manual\"")))
          {
            System.err.println(
              "K must be specified if not using the manual split strategy.");
            System.exit(1);
          }

          splitStrategy = FoldParser.SplitStrategy.manual;
        }
        else
        {
          argument = a;
          ...
          if (s.equals("random"))
            splitStrategy = FoldParser.SplitStrategy.random;
          else if (s.equals("sequential"))
            splitStrategy = FoldParser.SplitStrategy.sequential;
          else if (s.equals("kth"))
            splitStrategy = FoldParser.SplitStrategy.kth;
          else if (s.equals("manual"))
            splitStrategy = FoldParser.SplitStrategy.manual;
          else
          {
            System.err.println("Error: '" + s + "' is not a split strategy.");
            System.exit(1);
          }
        }
      }
    ...
    /**
      ...
      *           algorithm.
      * @param k  The number of folds for cross validation.
      * @param s  The data splitting strategy for cross validation.
      * @param al The width of the confidence interval for cross validation
      *           output.
      * @param p  The frequency in examples of progress updates.
     **/
    protected Clause(int t, ASTNode a, Constant r, Block b, Constant k,
                     FoldParser.SplitStrategy s, Constant al, Constant p)
    {
      type = t;
      argument = a;
      rounds = r;
    ...
    **/
    public Object clone()
    {
      ASTNode a = (ASTNode) argument.clone();
      Constant r = (Constant) rounds.clone();
      Block b = (Block) learnerParameterBlock.clone();
      Constant k = (Constant) K.clone();
      Constant al = (Constant) alpha.clone();
      Constant p = (Constant) progressOutput.clone();
      return new Clause(type, a, r, b, k, splitStrategy, al, p);
    }
